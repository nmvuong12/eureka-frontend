  ...
  <div class="class-list space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-gray-100">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">{{ $t('class.desc') }}</p>
      </div>
    </div>
    ...
    <el-card shadow="sm" class="border border-gray-100 rounded-2xl" :body-style="{ padding: '16px 24px' }">
      <div class="flex flex-wrap items-center gap-3">
        ...
        <el-input
          v-model="searchQuery"
          :placeholder="$t('class.search_placeholder')"
          clearable
          class="w-56"
        ...
        <el-select
          ...
          :placeholder="$t('class.filter_batch')"
          clearable
          filterable
          class="w-56"
          @change="handleSearch"
        ...
        <el-select
          ...
          :placeholder="$t('class.filter_pattern')"
          clearable
          filterable
          class="w-48"
          @change="handleSearch"
        ...
        <!-- Lọc trạng thái -->
        <el-select v-model="filterStatus" :placeholder="$t('common.status')" clearable class="w-40" @change="handleSearch">
          <el-option :label="$t('class.status_draft')" value="DRAFT" />
          <el-option :label="$t('class.status_enrolling')" value="ENROLLING" />
          <el-option :label="$t('class.status_rebalancing')" value="REBALANCING" />
          <el-option :label="$t('class.status_open')" value="OPEN" />
          <el-option :label="$t('class.status_studying')" value="STUDYING" />
          <el-option :label="$t('class.status_finished')" value="FINISHED" />
          <el-option :label="$t('class.status_cancelled')" value="CANCELLED" />
        </el-select>
    ...
    <el-card shadow="sm" class="border border-gray-100 rounded-2xl overflow-hidden" :body-style="{ padding: '0px' }">
      ...
      >
        <!-- Mã lớp -->
        <el-table-column prop="classCode" :label="$t('class.code')" width="150">
        ...
        <!-- Kế hoạch / Khóa học -->
        <el-table-column :label="$t('class.batch_course')" min-width="180">
          <template #default="{ row }">
        ...
        <!-- Lịch học -->
        <el-table-column :label="$t('class.schedule')" width="160">
          <template #default="{ row }">
            <span v-if="row.patternLabel || row.patternCode" class="text-sm text-teal-700 font-semibold">
              {{ row.patternLabel || row.patternCode }}
            </span>
            <span v-else class="text-gray-400 text-xs">{{ $t('class.not_assigned') }}</span>
          </template>
        </el-table-column>

        <!-- Giáo viên -->
        <el-table-column :label="$t('class.teacher')" width="160">
          <template #default="{ row }">
            <span v-if="row.teacherName" class="text-sm text-gray-700">{{ row.teacherName }}</span>
            <span v-else class="text-gray-400 text-xs italic">{{ $t('class.teacher_not_assigned') }}</span>
          </template>
        </el-table-column>

        <!-- Sĩ số -->
        <el-table-column :label="$t('class.student_size')" width="100" align="center">
          <template #default="{ row }">
        ...
        <!-- Ngày khai giảng -->
        <el-table-column :label="$t('class.start_date')" width="140" align="center">
          <template #default="{ row }">
        ...
        <!-- Thao tác -->
        <el-table-column :label="$t('common.actions_col')" width="140" fixed="right" align="center">
          <template #default="{ row }">
            <div class="flex items-center justify-center gap-1">
              <el-tooltip :content="$t('class.tooltip_edit')" placement="top">
                <el-button type="primary" link @click="openEditDialog(row)">
                   <el-icon :size="16"><Edit /></el-icon>
                </el-button>
              </el-tooltip>
              <el-tooltip :content="$t('class.tooltip_log')" placement="top">
                <el-button type="info" link @click="openLogDialog(row)">
                  <el-icon :size="16"><Document /></el-icon>
                </el-button>
              </el-tooltip>
              <el-tooltip :content="$t('class.tooltip_delete')" placement="top">
        ...
        <template #empty>
          <el-empty :description="$t('class.empty')" />
        </template>
      </el-table>

      <!-- Footer info -->
      <div class="flex justify-between items-center p-4 border-t border-gray-100 bg-gray-50/50">
        <div class="text-xs text-gray-500">{{ $t('class.show_count', { count: classes.length }) }}</div>
      </div>
    </el-card>
    ...
    <el-dialog
      ...
      :title="$t('class.log_title', { code: selectedClass?.classCode || '#' + selectedClass?.id })"
      width="60%"
      destroy-on-close
    >
      <div v-loading="logLoading">
        <el-table :data="classLogs" stripe class="w-full">
          <!-- Thời gian -->
          <el-table-column :label="$t('class.log_time')" width="160">
            <template #default="{ row }">
          ...
          <!-- Hành động -->
          <el-table-column prop="action" :label="$t('class.log_action')" width="140">
            <template #default="{ row }">
          ...
          <!-- Thay đổi trạng thái -->
          <el-table-column :label="$t('class.log_status')" width="200">
            <template #default="{ row }">
          ...
          <!-- Ghi chú -->
          <el-table-column prop="note" :label="$t('class.log_note')" min-width="150">
            <template #default="{ row }">
          ...
          <!-- Người thực hiện -->
          <el-table-column prop="createdBy" :label="$t('class.log_by')" width="130">
            <template #default="{ row }">
          ...
          <template #empty>
            <el-empty :description="$t('class.log_empty')" :image-size="60" />
          </template>
        </el-table>
      </div>
      <template #footer>
        <el-button @click="showLogDialog = false">{{ $t('common.close') }}</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="$t('class.edit_title')"
      width="500px"
      destroy-on-close
      class="premium-dialog"
    >
      <el-form :model="editForm" label-position="top">
        <el-form-item :label="$t('class.edit_name')" required>
          <el-input v-model="editForm.name" :placeholder="$t('class.edit_name')" />
        </el-form-item>
        
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item :label="$t('class.edit_max_students')">
              <el-input-number
          ...
          <el-col :span="12">
            <el-form-item :label="$t('class.edit_teacher')">
              <el-select
                v-model="editForm.teacherId"
                :placeholder="$t('class.edit_teacher_select')"
                clearable
                filterable
                class="w-full"
              >
        ...
        <el-form-item :label="$t('class.edit_note')">
          <el-input
            v-model="editForm.note"
            type="textarea"
            :rows="3"
            :placeholder="$t('class.edit_note_placeholder')"
          />
        </el-form-item>
      </el-form>
      <template #footer>
...
const fetchClasses = async () => {
  ...
  try {
    ...
    ElMessage.error(t('class.error_load'));
  } finally {
    loading.value = false;
  }
};
...
const getClassStatusLabel = (status?: string) => {
  const map: Record<string, string> = {
    DRAFT: t('class.status_draft'),
    ENROLLING: t('class.status_enrolling'),
    REBALANCING: t('class.status_rebalancing'),
    OPEN: t('class.status_open'),
    STUDYING: t('class.status_studying'),
    FINISHED: t('class.status_finished'),
    CANCELLED: t('class.status_cancelled')
  };
...
const submitEditForm = async () => {
  ...
  if (!editForm.value.name.trim()) {
    ElMessage.warning(t('class.edit_name_required'));
    return;
  }
  ...
  try {
    ...
    if (res.success) {
      ElMessage.success(t('class.edit_success'));
      editDialogVisible.value = false;
      await fetchClasses();
    } else {
      ElMessage.error(res.message || t('class.edit_failed'));
    }
  } catch (err: any) {
    const errorMsg = err.response?.data?.message || t('class.edit_failed');
...
const handleDeleteClass = async (row: SchoolClass) => {
  ...
  try {
    await ElMessageBox.confirm(
      t('class.delete_confirm', { code: row.classCode || row.name }),
      t('class.delete_confirm_title'),
      {
    ...
    if (res.success) {
      ElMessage.success(t('class.delete_success'));
      await fetchClasses();
    } else {
      ElMessage.error(res.message || t('class.delete_failed'));
    }
  } catch (err: any) {
    if (err !== 'cancel') {
      const errorMsg = err.response?.data?.message || t('class.delete_failed');
      ElMessage.error(errorMsg);
    }
  }
};
